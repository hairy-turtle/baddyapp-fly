<template>
	<v-badge bottom right class="search-badge" :value="activeFilterCount" :disabled="!activeFilterCount || filterActive">
		...
		>
			...
			<input ref="input" :value="modelValue" :placeholder="t('search_items')" @keyup.esc="blankInput" @input="emitValue" @paste="emitValue" />
			<v-icon
				v-if="modelValue"
				clickable
				class="icon-empty"
				name="close"
				@click.stop="blankInput"
				/>
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		function disable() {
			// Instead of shrinking search bar when clicking outside the search box, we clear it instead.
			// TODO: There's a bug when blanking input on player selection, where the player won't get
			// selected and the input is dismissed.
			// blankInput();
